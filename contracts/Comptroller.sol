...
contract Comptroller is ComptrollerInterface{
    ...
    function exitMarket(address bTokenAddress) override external returns (uint) {
        ...
        (uint oErr, uint tokensHeld, uint borrowBalance, ) = bToken.getAccountInfo(msg.sender);
    ...
    function getHypotheticalAccountLiquidityInternal(
        ...
        for (uint i = 0; i < assets.length; i++) {
            ...
            (oErr, vars.bTokenBalance, vars.borrowBalance, vars.exchangeMantissa) = asset.getAccountInfo(account);
